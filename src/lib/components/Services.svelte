...
<IntersectionObserver id="services" classNames="w-full bg-greyBG py-32 relative px-10 md:px-32">
	...
	<SectionTitle show={inView} title="Services" />
		</div>
	<h4 class={`font-medium text-white my-14 text-md md:text-2xl ${inView?'opacity-100 translate-y-0':'opacity-0 translate-y-10'} transition-[opacity,transform] duration-700 ease-in-out delay-300`}>Our ability to deliver quality services has grown over the years because of experienced workforce.</h4> <div class="grid grid-cols-1 md:grid-cols-3 max-md:gap-y-10 md:gap-x-12">
